...
from distributed import gather_with_grad
from loss import clip_loss, grad_cache_loss
from models import BiEncoder, BiEncoderConfig, LogitScale

from .base import BaseTrainer


class TextTextTrainer(BaseTrainer):
    def __init__(self, config, dtype, args):
    ...
    def get_old_model(self, config):
        config = config.model_args
        self.print(f"Loading old model from {config.old_checkpoint}")
        model_config = BiEncoderConfig.from_pretrained(config.old_checkpoint)
        if config.projection_dim is not None:
            model_config.projection_dim = config.projection_dim
        if config.gradient_checkpointing:
            model_config.gradient_checkpointing = True
        model = BiEncoder.from_pretrained(config.old_checkpoint, config=model_config)
    ...
    def forward_step(self, model, old_model, inputs, use_kd, logit_scale, **kwargs):
        model.train()
    ...
    def get_query_outputs(self, batch, model, **kwargs):
        query_outputs = model(
    ...
    def get_document_outputs(self, batch, model, **kwargs):
    ...
    def _forward_step(self, model, old_model, batch, use_kd, logit_scale, matryoshka_dims=None, matroyshka_loss_weights=None, **kwargs):
        ...
        if matryoshka_dims:
            ...
            if use_kd:
                with torch.no_grad():
                    docs_old = self.get_document_outputs(batch, **old_model)
                    queries_old = self.get_query_outputs(batch, **old_model)

                qfd_loss = F.cosine_embedding_loss(
                        queries, queries_old.detach(), torch.ones(queries.shape[0]).to(model.device)
                        )*qfd
                dfd_loss = F.cosine_embedding_loss(
                        docs, docs_old.detach(), torch.ones(docs.shape[0]).to(model.device)
                        )*dfd
            
            
                loss = loss + qfd_loss + dfd_loss

        return loss


    def training_step(
        self, model, old_model, batch, use_kd, optimizer, scheduler, step, train_args, total_num_steps, gradient_accumulation_steps
    ):
