...
from dataset.text_text_loader import StreamingShardDataset
from distributed import DistributedWandbTracker, gather, print_rank_zero
...
class BaseTrainer(metaclass=ABCMeta):
    def __init__(self, config, dtype=torch.float32):
        ...
        if config.model_args.old_checkpoint is not None:
            self.old_model = self.get_old_model(config)
        else:
    ...
    @abstractmethod
    def forward_step(self, model, old_model, inputs, use_kd, **kwargs):
        pass
    ...
    def training_step(
        ...
        with torch.autocast(device_type="cuda", dtype=self.dtype, enabled=not self.deepspeed):
            loss = self.forward_step(inputs=batch, **model, old_model=old_model, step=step, use_kd=use_kd)
    ...
    def train(self):
        ...
        if task_num > 1:
            ...
            q_drift_ = []
            dd = 0
            self.print(f"Total number of steps: {total_num_steps}")
            progbar = tqdm(
                train_dataloader, disable=not self.global_rank == 0, total=total_num_steps
            )
            model["model"].eval()
            old_model["model"].eval()
            q_drift = torch.zeros(768)
            with torch.autocast(device_type="cuda", dtype=self.dtype, enabled=not self.deepspeed):
                with torch.no_grad():
                    for step, batch in enumerate(progbar):
                        q_2 = self.get_query_outputs(batch, **model)
                        q_1 = self.get_query_outputs(batch, **old_model)

                        # To compute a single drift for all embeddings
                        z = (q_2 - q_1).cpu()
                        q1, q2 = [], []
                        dd += z.shape[0]
                        q_drift += z.sum(0)
            ...
            os.makedirs("QDC_drift_vectors", exist_ok=True)
            with open(f'QDC_drift_vectors/query_drift_t{task_num}.pickle', 'wb') as handle:
                pickle.dump(q_drift, handle, protocol=pickle.HIGHEST_PROTOCOL)
                handle.close()
